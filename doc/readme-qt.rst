Number42-qt: Qt4 GUI for Number42
===============================
...
Alternatively, install Qt Creator and open the `Number42-qt.pro` file.

An executable named `Number42-qt` will be built.
...
A warning for people using the *static binary* version of Number42 on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of Number42 is linked against libdb5.3.
...
installed causes Number42-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
