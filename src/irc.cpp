...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #Number42TEST2\r");
            Send(hSocket, "WHO #Number42TEST2\r");
        } else {
            // randomly join #Number4200-#Number4205
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #Number42%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #Number42%02d\r", channel_number).c_str());
        }
